defmodule Cursoseti.Utils.CustomTelemetry do
  alias Cursoseti.Utils.DataTypeUtils
  import Telemetry.Metrics

  @service_name Application.compile_env!(:cursoseti, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("cursoseti-plug-stop", [:cursoseti, :plug, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("cursoseti-redis-stop", [:redix, :pipeline, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("cursoseti-vm-memory", [:vm, :memory], &handle_custom_event/4, nil)
  ...
  def handle_custom_event([:cursoseti, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
